...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (paymentIntent.status === 'succeeded') {
      ...
      // Idempotency at the DB level too: if a retried request already saved
      // this same payment, don't insert a second order row.
      const { data: existing } = await supabase.from('orders').select('order_number').eq('stripe_id', paymentIntent.id).limit(1);
      let orderNumber;
      if (existing && existing.length) {
        orderNumber = existing[0].order_number;
      } else {
        orderNumber = await saveOrder({ email, name, address, city, state, zip, items, amount, stripeId: paymentIntent.id });
